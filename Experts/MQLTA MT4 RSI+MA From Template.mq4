...
input int SLRecentCandles=5;                                      //Number of candles to consider for the Stop Loss value
...
void EvaluateEntry(){
   ...
   //Defining the data needed for the entry evaluation: RSI + MA
   double MACurr=iMA(Symbol(),PERIOD_CURRENT,MAPeriod,MAShift,MAMethod,MAAppliedPrice,1);    //MA Current is the MA value in the last closed candle (1)
   double RSICurr=iRSI(Symbol(),PERIOD_CURRENT,RSIPeriod,RSIAppliedPrice,1);                 //RSI Current is the RSI value in the last closed candle (1)
   double RSIPrev=iRSI(Symbol(),PERIOD_CURRENT,RSIPeriod,RSIAppliedPrice,2);                 //RSI Current is the RSI value before the last closed candle (2)
   ...
   //If the Price of the closed candle is above MA and RSI goes from oversold to range then BUY
   if(iClose(Symbol(),PERIOD_CURRENT,1)>MACurr && RSIPrev<RSIOversold && RSICurr>RSIOversold) SignalEntry=SIGNAL_ENTRY_BUY;
   ...
   //If the Price of the closed candle is below MA and RSI goes from overbought to range then SELL
   if(iClose(Symbol(),PERIOD_CURRENT,1)<MACurr && RSIPrev>RSIOverbought && RSICurr<RSIOverbought) SignalEntry=SIGNAL_ENTRY_SELL;
...
void EvaluateExit(){
   ...
   if(iClose(Symbol(),PERIOD_CURRENT,1)<MACurr || RSICurr>RSIOverbought) SignalExit=SIGNAL_EXIT_BUY;
   ...
   //Close Sell if Price closes above MA or RSI closes in oversold area
   if(iClose(Symbol(),PERIOD_CURRENT,1)>MACurr || RSICurr<RSIOversold) SignalExit=SIGNAL_EXIT_SELL;
...
void ExecuteTrailingStop(){
   ...
   for(int i=0;i<OrdersTotal();i++) {
      ...
      if(OrderType()==OP_BUY){
         ...
         //Defining the new stop loss as the lowest low of the last SLRecentCandles, including the current candle in formation
         NewSLPrice=iLow(Symbol(),PERIOD_CURRENT,iLowest(Symbol(),PERIOD_CURRENT,MODE_LOW,SLRecentCandles,0));
      ...
      if(OrderType()==OP_SELL){
         ...
         //This is where you should include the code to assign a new value to the STOP LOSS
         //Defining the new stop loss as the highest high of the last SLRecentCandles, including the current candle in formation
         NewSLPrice=iHigh(Symbol(),PERIOD_CURRENT,iHighest(Symbol(),PERIOD_CURRENT,MODE_HIGH,SLRecentCandles,0));
